...
# Global variables to store the pipeline, upsampler, and config
PIPELINE = None
...
def create_ltx_video_pipeline(
    ...
    assert os.path.exists(ckpt_path), f"Ckpt path {ckpt_path} does not exist"
...
def initialize_pipeline(pipeline_config_path: str = "ltx_video/configs/ltxv-13b-0.9.7-distilled.yaml"):
    global PIPELINE, LATENT_UPSAMPLER, PIPELINE_CONFIG
    if PIPELINE is not None:
        return  # Pipeline already initialized

    if not os.path.isfile(pipeline_config_path):
        raise ValueError(f"Pipeline config file {pipeline_config_path} does not exist")
    ...
    spatial_upscaler_model_name_or_path = PIPELINE_CONFIG.get("spatial_upscaler_model_path")
    ...
    prompt_enhancement_words_threshold = PIPELINE_CONFIG["prompt_enhancement_words_threshold"]
...
def infer(
    ...
    **kwargs,
    ...
    global PIPELINE, LATENT_UPSAMPLER, PIPELINE_CONFIG
    if PIPELINE is None or PIPELINE_CONFIG is None:
        initialize_pipeline(pipeline_config)
    ...
    pipeline_config_dict = PIPELINE_CONFIG.copy()
    del pipeline_config_dict["stg_mode"]
    ...
    images = PIPELINE(
        ...
        **sample,
        ...
        mixed_precision=(pipeline_config_dict["precision"] == "mixed_precision"),
...
def main():
    ...
    parser.add_argument(
        "--pipeline_config",
        type=str,
        default="ltx_video/configs/ltxv-13b-0.9.7-distilled.yaml",
